   ...
   if(channel == avtCameras[leftCameraID].lcmChannelName)
      {
         ...
         workingImage = cv::Mat(image->height, image->width, CV_8U, (void *)image->data.data());
         leftImage = workingImage.clone();
         if(storePNGs)
            {
               leftColorImage = cv::Mat(image->width, image->height, CV_8UC3);
         ...
         if(abs(leftTime - rightTime) < INTERIMAGE_CRITERIA)
            ...
            //   cv::imshow("rights",rightImage);
             ...
             //  cv::destroyWindow("right");
               ...
               if(storePNGs)
                  {
                     int numChars = makeTimeString(thePairTime,imageTimeString,recordingPrefix, "png");
                     cv::Mat stereoImage = cv::Mat(image->height, image->width*2, CV_8UC3);
                     cv::hconcat(leftColorImage,rightColorImage,stereoImage);
                     snprintf(imageName,767,"%s/%s",theDataDir,imageTimeString);
                     tags = {cv::IMWRITE_PNG_COMPRESSION, pngCompression};
               ...
               else
                  {
                     ...
                     cv::Mat stereoImage = cv::Mat(image->height, image->width*2, CV_8U);
...
void *stereoLoggingThread (void *)
{
   ...
   if(!okINI)
   ...
   else
      {
         ...
         pngCompression = iniFile->readInt("GENERAL","PNG_COMPRESSION",DEFAULT_PNG_COMPRESSION);
         storePNGs = (bool)iniFile->readInt("GENERAL","STORE_PNGS",1);
         for(int cameraNumber = 0; cameraNumber < nOfAvtCameras; cameraNumber++)
            {
               char cameraLabel[32];
